...
This tool acts as a client to the `vm-instance-rot`. It listens for nonces sent
from a challenger. These nonces are then combined with a blob of data to create
the `qualifying_data` that's sent down to the `vm-instance-rot` where they're
included in an attestation from the Oxide Platform RoT.

### appraiser

The appraiser sends challenges / nonces to the `vm-instance`.
It receives a `PlatformAttestation` in response.
This attestation is then appraised.
...
In this configuration all components on a single host OS. The `vm-instance-rot`
and the `vm-instance` communicate over a unix domain socket. The `appraiser`
sends challenges to the `vm-instnace` over TCP.
